...
ORDER BY
 ...
 -- finding the number of non-round trips per route
...
-- finding the average ride_length

SELECT
  CAST(CAST(AVG(CAST(CAST(ride_length as datetime) as float)) as datetime) as time) avg_ride_length
FROM
  bikeshare
...
--finding the average ride_length of round trips

SELECT
  CAST(CAST(AVG(CAST(CAST(ride_length as datetime) as float)) as datetime) as time) avg_round_trip_dur 
FROM
  bikeshare
WHERE
  start_station_name = end_station_name;
...
--finding the average ride_length of non-round trips

SELECT
  CAST(CAST(AVG(CAST(CAST(ride_length as datetime) as float)) as datetime) as time) avg_non_round_trip_dur 
FROM
  bikeshare
WHERE
  start_station_name != end_station_name;
...
-- finding the average ride_length per member_casual and weekday

SELECT
  member_casual,
  weekday,
  CAST(CAST(AVG(CAST(CAST(ride_length as datetime) as float)) as datetime) as time) avg_ride_length_per_members 
FROM
  bikeshare
GROUP BY
  member_casual,
  weekday
ORDER BY
  avg_ride_length_per_members DESC;
